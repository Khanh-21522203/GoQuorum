...
var _ adapter.Transport = (*Client)(nil)
...
func (c *Client) RemotePut(id node.NodeID, key []byte, siblings *adapter.SiblingSet, done func(error)) {
	done(contracts.ErrNotImplemented)
}

// RemoteGet reads a key's sibling set from node id.
func (c *Client) RemoteGet(id node.NodeID, key []byte, done func(*adapter.SiblingSet, error)) {
...
func (c *Client) GossipExchange(id node.NodeID, entries []adapter.GossipEntry, done func([]adapter.GossipEntry, error)) {
	done(nil, contracts.ErrNotImplemented)
}
